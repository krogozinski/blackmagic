...
static void lpc43x0_spi_read(target *t, uint32_t command, target_addr address, void *buffer, size_t length);
static void lpc43x0_spi_write(target *t, uint32_t command, target_addr address, const void *buffer, size_t length);
static bool lpc43x0_spi_mass_erase(target *t);
static int lpc43x0_spi_flash_erase(target_flash_s *f, target_addr addr, size_t length);
static int lpc43x0_spi_flash_write(target_flash_s *f, target_addr dest, const void *src, size_t length);
...
static void lpc43xx_detect(target *const t, const lpc43xx_partid_s part_id)
{
...
static void lpc43x0_spi_read_sfdp(target *const t, const uint32_t address, void *const buffer, const size_t length)
{
	lpc43x0_spi_read(t, SPI_FLASH_CMD_READ_SFDP, address, buffer, length);
}

static void lpc43x0_add_spi_flash(target *const t, const size_t length, const uint32_t read_command)
{
...
static void lpc43x0_detect(target *const t, const lpc43xx_partid_s part_id)
{
	switch (part_id.part) {
...
static void lpc43x0_spi_read(
	target *const t, const uint32_t command, const target_addr address, void *const buffer, const size_t length)
{
...
static void lpc43x0_spi_write(
	target *const t, const uint32_t command, const target_addr address, const void *const buffer, const size_t length)
{
	if ((command & LPC43x0_SPIFI_FRAME_MASK) != LPC43x0_SPIFI_FRAME_OPCODE_ONLY)
		target_mem_write32(t, LPC43x0_SPIFI_ADDR, address);
	target_mem_write32(t, LPC43x0_SPIFI_CMD, command | LPC43x0_SPIFI_DATA_LENGTH(length));
...
static inline uint8_t lpc43x0_spi_read_status(target *const t)
{
	uint8_t status;
...
static void lpc43x0_spi_run_command(target *const t, const uint32_t command)
{
...
{
	target *const t = f->t;
	const target_addr begin = dest - f->start;
