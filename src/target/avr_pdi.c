...
bool avr_pdi_st8(avr_pdi_t *pdi, uint32_t reg, uint8_t data)
{
	...
	if (avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, command) || result != PDI_EMPTY ||
		...
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, data))
		return false;
	return result == PDI_EMPTY;
}

uint8_t avr_pdi_ld8(avr_pdi_t *pdi, uint32_t reg)
{
	...
	if (avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, command) || result != PDI_EMPTY ||
		...
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (reg >> 16U) & 0xffU) || result != PDI_EMPTY ||
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (reg >> 24U) & 0xffU) || result != PDI_EMPTY ||
		!avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, data, 0))
		return 0xFFU; // TODO - figure out a better way to indicate failure.
	return data[0];
}

bool avr_pdi_st16(avr_pdi_t *pdi, uint32_t reg, uint16_t data)
{
	...
	if (avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, command) || result != PDI_EMPTY ||
		...
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (reg >> 16U) & 0xffU) || result != PDI_EMPTY ||
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (reg >> 24U) & 0xffU) || result != PDI_EMPTY ||
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, data & 0xffU) || result != PDI_EMPTY ||
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (data >> 8U) & 0xffU))
		return false;
	return result == PDI_EMPTY;
}

uint16_t avr_pdi_ld16(avr_pdi_t *pdi, uint32_t reg)
{
	...
	if (avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, command) || result != PDI_EMPTY ||
		...
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (reg >> 16U) & 0xffU) || result != PDI_EMPTY ||
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (reg >> 24U) & 0xffU) || result != PDI_EMPTY ||
		!avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, data + 0, 0) ||
		!avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, data + 1, 0))
		return 0xFFFFU; // TODO - figure out a better way to indicate failure.
	return data[0] | (data[1] << 8U);
}

bool avr_pdi_st32(avr_pdi_t *pdi, uint32_t reg, uint32_t data)
{
	...
	if (avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, command) || result != PDI_EMPTY ||
		...
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (reg >> 24U) & 0xffU) || result != PDI_EMPTY ||
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, data & 0xffU) || result != PDI_EMPTY ||
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (data >> 8U) & 0xffU) || result != PDI_EMPTY ||
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (data >> 16U) & 0xffU) || result != PDI_EMPTY ||
		avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, (data >> 24U) & 0xffU))
		return false;
	return result == PDI_EMPTY;
}

uint32_t avr_pdi_ld32(avr_pdi_t *pdi, uint32_t reg)
{
	...
	if (avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, &result, command) || result != PDI_EMPTY ||
		...
		!avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, data + 0, 0) ||
		!avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, data + 1, 0) ||
		!avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, data + 2, 0) ||
		!avr_jtag_shift_dr(&jtag_proc, pdi->dp_jd_index, data + 3, 0))
		return 0xFFFFU; // TODO - figure out a better way to indicate failure.
	return data[0] | (data[1] << 8U) | (data[2] << 16U) | (data[3] << 24U);
}

bool avr_pdi_read(avr_pdi_t *pdi, uint32_t addr, uint8_t ptr_mode, void *dst, uint32_t count)
{
...
bool avr_enable(avr_pdi_t *pdi, pdi_key_e what)
{
...
bool avr_disable(avr_pdi_t *pdi, pdi_key_e what)
{
...
static void avr_regs_read(target *t, void *data)
{
	...
	regs->pc = avr_pdi_ld32(pdi, AVR_ADDR_DBG_PC);
	regs->sp = avr_pdi_ld16(pdi, AVR_ADDR_CPU | AVR_ADDR_CPU_SPL);
	regs->sreg = avr_pdi_ld16(pdi, AVR_ADDR_CPU | AVR_ADDR_CPU_SREG);

	if (!avr_pdi_st8(pdi, AVR_ADDR_DBG_CTRL, AVR_DBG_READ_REGS) ||
		!avr_pdi_st32(pdi, AVR_ADDR_DBG_CTR, AVR_NUM_REGS) ||
		!avr_pdi_reg_write(pdi, PDI_REG_R4, 1) ||
		!avr_pdi_read(pdi, AVR_ADDR_DBG_SPECIAL, PDI_MODE_IND_PTR, regs->general, 32))
