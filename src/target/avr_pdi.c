...
static bool avr_pdi_read_ind(avr_pdi_t *pdi, uint32_t addr, uint8_t ptr_mode, void *dst, uint32_t count)
{
...
static void avr_regs_read(target *t, void *data)
{
	...
	if (!avr_pdi_read32(pdi, AVR_ADDR_DBG_PC, &pc) ||
		!avr_pdi_read16(pdi, AVR_ADDR_CPU | AVR_ADDR_CPU_SPL, &sp) ||
		!avr_pdi_read8(pdi, AVR_ADDR_CPU | AVR_ADDR_CPU_SREG, &regs->sreg) ||
